...
**Domain:** `Finance / Accounts Payable` · **Suite:** `Finance Document Suite` · **Accent:** `#67e8f9`
...
**Invoice Ai** is a standalone, browser-based AI workflow app for **Finance / Accounts Payable**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `invoice_file`<br>Invoice file | text | Yes | — | Affects input: Invoice file. |
...
| `country`<br>country | select | No | United States | Affects customization: country. |
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `tax_rules`<br>tax rules | textarea | No | — | Affects customization: tax rules. |
| `vendor_list`<br>vendor list | text | No | — | Affects customization: vendor list. |
| `po_tolerance`<br>PO tolerance | slider | No | 50 | Affects customization: PO tolerance. |
| `approval_levels`<br>approval levels | select | No | manual review | Affects customization: approval levels. |
| `export_schema`<br>export schema | select | No | — | Affects customization: export schema. |
...
- **country**: United States, United Kingdom, Egypt, UAE, Saudi Arabia, Germany, Custom
- **currency**: USD, EUR, GBP, EGP, AED, SAR, USDT
- **approval levels**: manual review, manager approval, auto approve low risk, custom
...
uvicorn app.main:app --host 127.0.0.1 --port 9128
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9128`. |
...
- Export download
- Job history
- ERP/accounting optional
- Tax/VAT database optional
- OCR vision optional
...
```bash
curl http://127.0.0.1:9128/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9128/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9128 uvicorn app.main:app --host 0.0.0.0 --port 9128
```
...
- `plugins/invoice-ai.json` for plugin metadata.
